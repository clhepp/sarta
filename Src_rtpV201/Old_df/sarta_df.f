...
C 09 May 2008 Scott Hannon   Add freq adjustment code including A & B
C                            variables, two calls to rdcoef & tunmlt,
C                            new call to sumcof, and add yoff to rdrtp.
C 14 May 2008 Scott Hannon   Add default freq FRQDEF and WGTA,WGTB; add
C                               CO2TOP to calpar.f & calnte.f calls;
C                               add CO2PPM to calpar.f call; move no
C                               prof CO2MLT calc to calpar.f; add AORB
C                               to RDSUN
C 24 Oct 2008 Scott Hannon   Update for rtpV201
...
C      Read the "A" coef database
     ...
     $    NCHCO2, INDCO2, COFCOA, NCHN2O, INDN2O, COFN2A,
     $    NCHSO2, INDSO2, COFSOA, NCHHNO, INDHNO, COFHNA,
     $    NCHH2O, INDH2O,  WAZOP, WAVGOP, COFH2A,
     $     FREQA, COEFFA, NCHNTE, CLISTN, COEFNA,     FX)
C
C      Get and apply multipler tuning to "A" coefficients
...
C      Read the "B" coef database
       AORB='B'
       CALL RDCOEF (AORB,   IOUN, NCHAN, INDCHN, SETCHN,
     $     NCHN1,  NCHN2,  NCHN3,  NCHN4,  NCHN5,  NCHN6,  NCHN7,
     $    CLIST1, CLIST2, CLIST3, CLIST4, CLIST5, CLIST6, CLIST7,
     $    COEF1B, COEF2B, COEF3B, COEF4B, COEF5B, COEF6B, COEF7B,
     $    NCHCO2, INDCO2, COFCOB, NCHN2O, INDN2O, COFN2B,
     $    NCHSO2, INDSO2, COFSOB, NCHHNO, INDHNO, COFHNB,
     $    NCHH2O, INDH2O,  WAZOP, WAVGOP, COFH2B,
     $     FREQB, COEFFB, NCHNTE, CLISTN, COEFNB,     FX)
C
C      Get and apply multipler tuning to "B" coefficients
       CALL TUNMLT( AORB,   IOUN,  NCHAN, INDCHN, SETCHN,
     $     NCHN1,  NCHN2,  NCHN3,  NCHN4,  NCHN5,  NCHN6,  NCHN7,
     $    CLIST1, CLIST2, CLIST3, CLIST4, CLIST5, CLIST6, CLIST7,
     $    COEF1B, COEF2B, COEF3B, COEF4B, COEF5B, COEF6B, COEF7B,
...
C
C      Calc summing weights for default freq
       WGTA=(YOFDEF - YOFFB)/(YOFFA - YOFFB)
       WGTB=(YOFDEF - YOFFA)/(YOFFB - YOFFA)
C
       IF (FCHAN(1) .GT. 640 .AND. FCHAN(1) .LT. 2670) THEN
          DO I=1,NCHAN
             FRQDEF=FREQA(I)*WGTA + FREQB(I)*WGTB
             RJUNK1=ABS(FRQDEF - FCHAN(I))
             RJUNK2=0.03*FRQDEF/1200.0   ! ~3% of a channel fullwidth
...
C      -------------------
C      Do freq adjustments
C      -------------------
       IF (YOFF .NE. YOFPRE) THEN

ccc for testing
c      print *,'IPROF,YOFF,YOFPRE=',IPROF,YOFF,YOFPRE
ccc
...
C
C         Update YOFPRE for the newly summed coef data
          YOFPRE=YOFF
       ENDIF
C
